...
//mt19937 rnd(1337);
//
//tuple<Fixed, bool, pair<int, int>> propagate_flow(int x, int y, Fixed lim, int leftXBoarder, int rightXBoarder) {
//    last_use[x][y] = UT - 1;
//    Fixed ret = 0;
...
//                return {vp, 1, {nx, ny}};
//            }
//            if (nx < leftXBoarder || x > rightXBoarder) {
//                continue;
//            }
//            auto [t, prop, end] = propagate_flow(nx, ny, vp, leftXBoarder, rightXBoarder);
//            ret += t;
//            if (prop) {
...
//int dirs[N][M]{};
//
//size_t THREAD_POOL_SIZE = 50;
//
//void parallel_propagate_flow() {
//    vector<future<void>> tasks;
//    size_t rows_per_thread = (N + THREAD_POOL_SIZE - 1) / THREAD_POOL_SIZE;
//
//    for (size_t t = 0; t < THREAD_POOL_SIZE; ++t) {
//        size_t start_row = t * rows_per_thread;
//        size_t end_row = min(start_row + rows_per_thread, N);
//
//        tasks.emplace_back(async(launch::async, [start_row, end_row]() {
//            for (size_t x = start_row; x < end_row; ++x) {
//                for (size_t y = 0; y < M; ++y) {
//                    if (field[x][y] != '#' && last_use[x][y] != UT) {
//                        auto [t, local_prop, _] = propagate_flow(x, y, 1, start_row, end_row);
//                        if (t > 0) {
...
//    std::cout << "Duration time: " << duration.count() << " seconds" << std::endl;
//}
